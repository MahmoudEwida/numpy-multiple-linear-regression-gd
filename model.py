...
def gd_step(X, y, weights, lr):
    ...
    weights -= lr * mse_gradient(X,y,y_pred)
    return weights
...
def epoch_train_val_losses(X_train, y_train, X_val, y_val, weights):
    ...
    train_loss = mse_loss(y_train,predict_linear(X_train,weights))
    val_loss = mse_loss(y_val,predict_linear(X_val,weights))
...
def run_one_epoch(state, X_train, y_train, X_val, y_val, lr, patience):
    ...
    # Unpack all 4 updated values:
    new_best_loss, new_wait, new_best_w, stop_flag = update_early_stop_state(
        val_loss,
        state["best_val_loss"],
        state["wait"],
        state["weights"],
    ...
    # Put them back into the backpack:
    state["best_val_loss"] = new_best_loss
    state["wait"] = new_wait
    state["best_weights"] = new_best_w
    state["stopped"] = stop_flag
...
def train_batch_gd(
    X_train, y_train, X_val, y_val, lr, epochs, patience, seed=None
):
    ...
    for _ in range(epochs):
        state = run_one_epoch(
            state, X_train, y_train, X_val, y_val, lr, patience
        )
...
def predict_lr_model(model, X):
    ...
    X_tr_design = prepare_design_matrix(X, model['mean'], model['std'])
